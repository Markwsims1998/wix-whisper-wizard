...
import { Search, Filter, User, MoreHorizontal, Shield, Ban } from "lucide-react";
import { 
  DropdownMenu, 
  DropdownMenuContent, 
  DropdownMenuItem, 
...
const mockUsers = Array(25).fill(null).map((_, index) => ({
  ...
  role: index < 3 ? 'admin' : (index < 8 ? 'moderator' : 'user'),
  status: index % 10 === 0 ? 'banned' : 'active',
  subscription: ['free', 'bronze', 'silver', 'gold'][index % 4],
  lastActive: new Date(Date.now() - Math.floor(Math.random() * 10) * 24 * 60 * 60 * 1000).toLocaleDateString(),
  joined: new Date(Date.now() - Math.floor(Math.random() * 365) * 24 * 60 * 60 * 1000).toLocaleDateString(),
  avatar: `https://randomuser.me/api/portraits/${index % 2 ? 'men' : 'women'}/${index % 70}.jpg`
}));
                ...
                <tbody className="[&_tr:last-child]:border-0">
                  {filteredUsers.map(user => (
                    <tr key={user.id} className="border-b transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      ...
                      <td className="p-4 align-middle">
                        <Badge className={
                          user.subscription === 'free' ? 'bg-gray-100 text-gray-800 hover:bg-gray-200' : 
                          user.subscription === 'bronze' ? 'bg-amber-700 text-white hover:bg-amber-800' : 
                          user.subscription === 'silver' ? 'bg-gray-400 text-white hover:bg-gray-500' : 
                          'bg-yellow-500 text-white hover:bg-yellow-600'
                        }>
                          {user.subscription.charAt(0).toUpperCase() + user.subscription.slice(1)}
                        </Badge>
                      </td>
                      <td className="p-4 align-middle text-sm">{user.lastActive}</td>
                      <td className="p-4 align-middle text-sm">{user.joined}</td>
                      <td className="p-4 align-middle text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {user.status === 'active' ? (
                              <DropdownMenuItem onClick={() => handleBanUser(user.id, user.name)} className="text-red-500 focus:text-red-500">
